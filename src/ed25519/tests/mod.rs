...
fn sm_test<F: BigPrimeField>(
    ...
    let sm = ecc_chip.scalar_mult::<Ed25519Affine>(
        ctx,
        P,
        s.limbs().to_vec(),
        fq_chip.limb_bits,
...
fn test_ed25519_sm_minus_1() {
    ...
    let base = Ed25519Affine::random(OsRng);
    let mut s = -Fq::one();
